...
6. ~~Not wired into `app.js` yet.~~ Deliberately still not: a standalone
   test harness (`web/v2/search.js` + `search.html`, served at `/v2/search`
   — see server.py) exists instead, kept separate on purpose while the
   design is still being shaken out. See the 2026-09-10 update below for
...
## Update 2026-09-10: a test harness, and two bugs only real end-to-end testing caught

`web/v2/search.js` (a standalone module -- imports nothing from and is imported by nothing in `app.js`) plus `web/v2/search.html` (a self-contained page, no dependency on `styles.css`) exist now, served at `/v2/search` via a small route in `server.py` that maps it onto the static file the same way `/logs` already does. Not linked from the real viewer -- purely a test harness for shaking out this design before it goes anywhere near production. Both bugs below were found by actually fetching real queries through the live server, not by unit-testing either side of the encoding contract in isolation -- worth remembering next time this feels "obviously correct" from reading the code alone.

**Bug 1: shard files were unreachable over HTTP for any prefix with a non-alphanumeric character -- 61% of them (127,261 of 207,225).** The original `shard_filename()` percent-encoded the prefix for *both* the on-disk filename and the (not-yet-written) client URL, on the theory that both sides just needed to "agree on an encoding." They don't need to agree on an encoding -- percent-encoding belongs only on the wire. Confirmed straight from the stdlib: `http.server.SimpleHTTPRequestHandler.translate_path` calls `urllib.parse.unquote()` on the *entire* request path before ever splitting it into filesystem components, so a client fetching the properly percent-encoded URL for e.g. `"(hed) Planet Earth"` arrives at the filesystem lookup already decoded back to `(hed) planet earth.json` -- but the file was written under its *encoded* name, `%28hed%29...json`, so it never matched. Fixed by writing shards under their raw, unencoded prefix and percent-encoding only in `search.js`'s URL construction (`shardFilename`) -- see `shard_search_index.py`'s updated `shard_filename` docstring for the detail. One consequence: `"/"` can never be part of a filesystem path component no matter how it's escaped (a decoded `/` is always a directory separator to `translate_path`, encoded-then-decoded or not), and real prefixes do contain it (`"AC/DC"`). Substituted with `SLASH_SUBSTITUTE`, which itself needed a second attempt: the first choice, U+2044 FRACTION SLASH, collided with real data and crashed the very next full rebuild (`AssertionError: duplicate shard filename: '⁄h.json'`) -- Unicode NFKD decomposition, which `normalise()` applies *before* the substitution runs, expands a vulgar fraction like `"½"` into `"1⁄2"`, so that "distinct" character was already reachable from ordinary text. Landed on U+E000 (Private Use Area), which has no decomposition and no meaning in real text by definition.

**Bug 2: a single-hit `findShard` returned zero results for many real multi-character queries, e.g. "bjork".** The original client logic tried prefix lengths in order and stopped at the first one that wasn't a 404. That's wrong whenever a bucket gets split: rows whose *whole* normalised name equals the prefix exactly (a real artist literally named "BJ") are written as their own small leaf at the shallow depth *alongside* the deeper files split for longer names sharing that prefix -- not nested under them. Concretely: `bj.json` exists (2 rows, entities literally named "BJ"), `bjo.json` exists (2 more), `bjor.json` doesn't exist, `bjork.json` exists (65 rows -- Björk and friends). Stopping at `bj.json` silently returned nothing for a query that had a real answer two characters deeper. Fixed by renaming to `findShards` (plural) and fetching *every* producible prefix length in parallel, merging whichever ones hit before filtering by the full query -- see its docstring in `search.js` for the detail, including why paying for a few extra 404s per query is the right trade against silently dropping real matches.

**Known limitation, not a bug, left in the code's docstring:** this index only ever does one contiguous prefix match against a node's own name. `"goldie timeless"` -- literally the motivating example in `CLAUDE.md`'s Known Issues for why `entitySearch` needs `_rank_matches`' neighbour-boost -- finds nothing here, because Goldie's 1995 album *Timeless* doesn't itself start with "goldie timeless"; recognising that needs the query split into terms and reasoned about across `CREDITED`, and this flat `(type, id, name, degree)` index carries no relationship data at all to reason with. Step 5 above (neighbour-boost's client-side replacement) is exactly this, still open.

## Update 2026-09-11: disambiguating same-titled releases, and the real fix this is standing in for

Displaying several different releases all titled "Timeless" side by side with no way to tell them apart (a real usability problem hit while testing `/v2/search`, not a hypothetical) needed `export_search_index.py` to carry two more Release-only fields:

- **`date`** -- `n.year` verbatim (misleadingly named on the Release node itself; it's usually a full date like `"2023-12-28"`, not just a year -- see the export script's docstring).
- **`artists`** -- up to `RELEASE_ARTIST_CAP` credited names, semicolon-joined. Getting this *right* ran into the same underlying gap as the `entitySearch` ranking problem above, one level down: **`CREDITED` carries no property distinguishing Discogs' `<artists>` (the headline credit) from `<extraartists>` (session/production credits)** -- `discogs_to_neo4j.py`'s `process_release` has always written both to the same relationship type with nothing to tell them apart, since the very first version of the transform. A release credited to the band Mucc plus ~50 backing musicians -- individually `Artist` nodes, not the `Group` -- surfaced "Yoshio Arimatsu; Jun-ichi Yajima; ..." (session players) under a plain `collect()` with no ordering to fall back on, not the band.

Two heuristics stand in for the real fix, described in full in the export script's docstring: prefer `Group`-labelled credits over `Artist`-labelled ones whenever any exist (fixes the Mucc case directly, and is also the cheaper query -- filtering out the session players before computing degree on any of them, rather than after, took the Release pass from ~1,062 rows/s to ~3,976 rows/s), then sort whichever pool that leaves by degree descending (handles multiple credited groups, and the majority of releases which have no group credit at all).

The real fix is upstream of the search index entirely -- `discogs_to_neo4j.py` should parse `<artists>` and `<extraartists>` as the distinct credit roles they actually are, rather than this leaning on degree-based guesswork. Tracked as a standing TODO, not buried in this session's notes: see `notes/TODO.md`.
